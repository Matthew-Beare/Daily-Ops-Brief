...
| Installable provider-neutral Life Planner skill and deterministic Personal Google bootstrap | CURRENT REQUIRED | Executable installable skill, module-scoped Sheets/Drive blueprint, plan/readback validator, and CI failure tests; live template/provider smoke evidence remains a release gate | Ship a self-contained new-user skill, create only selected Google modules, seed canonical registries/Interview Ledger, reject source/provider/identity/header drift, and never install the reference deployment skill for another user. |

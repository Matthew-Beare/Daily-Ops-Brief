...
| --- | --- | --- | --- |
| Exactly two briefs at 2:45 AM and 2:45 PM `America/New_York` | REQUIRED | Automation metadata repaired; next real firing still unproven | Keep one schedule, named TZID, provider readback, slot grace, Run Log proof. |
...
| Canonical-clock guard with DST-safe slot matching and bounded dispatch grace | REQUIRED by failure evidence | Repaired executable; boundary/DST/naive/offset/CLI tests pass | Retain actual-firing proof as a separate live scheduler gate. |

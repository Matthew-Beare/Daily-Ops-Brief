...
(() => {
  ...
  loadProviders = async function loadProviderReadiness() {
    ...
    if (byId("providerStatus")) byId("providerStatus").textContent = JSON.stringify({configured,health},null,2);
  };

  function sameResource(urlValue, resource) {
    if (!urlValue) return false;
    try {
      const url = new URL(urlValue, location.href);
      return url.pathname.endsWith(`/${resource}`) || url.pathname === resource || url.pathname.endsWith(resource);
    } catch (_) {
      return String(urlValue).split("?")[0].endsWith(resource);
    }
  }

  function loadScriptOnce(src, datasetKey, onload = null) {
    const existing = [...document.scripts].find((script) => script.hasAttribute(`data-${datasetKey}`) || sameResource(script.getAttribute("src"), src));
    if (existing) { if (onload) queueMicrotask(onload); return existing; }
    const script=document.createElement("script");
    script.src=src;
    script.setAttribute(`data-${datasetKey}`,"true");
    if (onload) script.onload=onload;
    script.onerror=()=>{ try { setStatus(`MIRA could not load ${src}. Refresh the app; if it happens again this build is not release-ready.`); } catch (_) {} };
    document.head.append(script);
    return script;
  }

  function loadStyleOnce(href, datasetKey) {
    const existing = [...document.querySelectorAll("link[rel='stylesheet']")].find((link) => link.hasAttribute(`data-${datasetKey}`) || sameResource(link.getAttribute("href"), href));
    if (existing) return existing;
    const css=document.createElement("link"); css.rel="stylesheet"; css.href=href; css.setAttribute(`data-${datasetKey}`,"true"); document.head.append(css); return css;
  }
  ...
  function loadCommercialShell() {
    ...
    loadStyleOnce("experience-v3.css","mira-experience-v3");
    if (globalThis.MirrorNative) loadStyleOnce("android-fixes.css","mira-android-fixes");

    loadScriptOnce("receipt-queue-cloud-v1.js","mira-receipt-queue-cloud");
    loadScriptOnce("reconciliation-cloud-v1.js","mira-reconciliation-cloud");
    ...
    loadScriptOnce("reconciliation-v1.js","mira-reconciliation-ui");
    loadScriptOnce("feature-studio-reconciliation-v1.js","mira-feature-studio-reconciliation");
    loadScriptOnce("onboarding-polish.js","mira-onboarding-polish");
    loadScriptOnce("interaction-audit.js","mira-interaction-audit");
    loadScriptOnce("experience-v3.js","mira-experience-v3-script");
    loadScriptOnce("native-kiosk-bridge.js","mira-native-kiosk-bridge");
  }

  document.addEventListener("DOMContentLoaded", loadCommercialShell);
  document.addEventListener("visibilitychange",()=>{ if(!document.hidden&&apiBase()) { loadProviders().catch(()=>{}); globalThis.MiraShell?.refreshHome?.().catch(()=>{}); globalThis.MiraExperienceV3?.refreshExperience?.().catch(()=>{}); globalThis.MiraReconciliationUI?.refresh?.().catch(()=>{}); } });
})();

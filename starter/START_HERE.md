...
- Reuse verified existing authorities and accessible current conversation, uploaded/File Library, Drive/email/calendar or other connected evidence.
- If a useful capability is missing, search supported integrations when possible. Do not invent access.
- Arbitrary old ChatGPT conversations are not guaranteed globally searchable. If useful prior-chat material is inaccessible, explain how to ingest it instead of pretending it was read.

Discover my actual life:
- Learn how my work/days operate, what creates friction, and what systems I already use.
...
- Reconcile accessible existing meal plans/recipes into one canonical recipe library rather than starting over.
...
- High-confidence evidence may update under the approved rule; ambiguity asks instead of guessing.
...
- meal planning/recipe library and appointment/email reconciliation when selected;
...
- A fresh conversation must recover from canonical authorities plus versioned source even after old chats are deleted.

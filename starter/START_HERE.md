...
Human entry point for a **non-technical user**. Read `STATE_AUTHORITY_MODEL.md`, `INTERVIEW_LEDGER.md`, `PROFILE_AND_CONTEXT_MODES.md`, `DEPENDENCIES.md`, `LIFE_INTERVIEW.md`, `MODULE_CATALOG.md`, `VERSIONING.md`, `PERSONAL_FORK_LIFECYCLE.md`, and `CAPABILITY_DISCOVERY.md` as needed. The user should not need JSON, Python, Git commands, database design, or automation jargon.
...
- Retired/not-working profiles bypass work-away machinery by default and may focus briefs on selected appointments, household/admin, family commitments, hobbies, travel, routines, projects, and documents.
...
- First boot explicitly records each as enabled, disabled, or unresolved.

...
Before Step 5, ask Codex to install and validate the `life-planner` skill from `starter/life-planner` in the new private repository. `life-planner` is a compatibility package ID; the product remains M.I.R.R.O.R. and the assistant remains MIRA. This is a browser/Codex action; the user does not download a ZIP, open a terminal, or paste commands. Codex must verify the installed skill contains `SKILL.md`, `assets/personal-google-blueprint.json`, and `scripts/google_bootstrap.py`. If the runtime cannot install a user skill, report:

...
class FeatureManifestTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.fixture = json.loads(
            (ROOT / "fixtures" / "features" / "meal-planning.feature.json").read_text(
                encoding="utf-8"
            )
        )
        cls.schema = json.loads(
            (ROOT / "schemas" / "feature-manifest.schema.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_personal_data_in_shared_source_is_rejected(self):
        ...
        self.assertIn(
            "source_contains_personal_data must be false",

...
- Exactly one active `LyfeOS Control Cycle` automation at 2:45 AM/PM Eastern. It runs the receipt/order lifecycle, the PM qualified-job watch, and the Ops Brief as module-isolated phases and returns one brief. No separate active Ops/lifecycle/job-watch, per-order, child, retry, 3:00, UTC, or Pacific duplicates.
...
- **Scheduled runtime time authority is the IANA canonical clock.** Convert the current offset-aware instant through `ZoneInfo("America/New_York")` and compare it with the configured slot and bounded grace window. Never use the travel/device/session clock or a hand-maintained UTC offset as the scheduled-entry gate. Scheduled control-cycle entry requires canonical `02:45` or `14:45`. A scheduled entry outside the bounded grace window is a scheduler-integrity failure and must stop downstream state-changing modules under the Module Circuit Breaker Report boundary. Manual runs are not rejected merely for occurring outside a scheduled slot.

...
The consolidated Receipt & Order Lifecycle may surface a proposed-contact approval as its run output, allowing the platform to notify the user. It must show the proposed recipient and message and ask for approval. It may not send during the scheduled run.

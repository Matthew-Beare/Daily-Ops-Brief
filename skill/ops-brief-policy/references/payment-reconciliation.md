...
`Awaiting Settlement` is a valid open state, not an error. The consolidated Receipt & Order Lifecycle rechecks it on every run using current account evidence. There is no per-order reminder or automation.
